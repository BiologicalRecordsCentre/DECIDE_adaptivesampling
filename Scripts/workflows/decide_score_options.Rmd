...
It makes most sense to use the multiplicative method of combining probability of presence and variation layers, so this is what I've used for the rest of the plots.

Combining metrics across species using some kind of weighting seems like a good option. Ideally, we want to combine them in such a way that:
...
We have decided to produce a metric which is evaluated across species; this will be easier for any user to digest and does not leave us open to too much criticism in terms of recorders claiming the models are wrong. Multiple potential options to test regarding how to combine the multiplied score across species to create a 'DECIDE score':
...
Want to compare all these possibilities using both the arithmetic mean and geometric mean to see which looks better.
...
```{r all_ps, fig.height = 7, fig.width=20, meassage = F, warning=F, include = TRUE}
...
```{r geo_mean_const, warning = FALSE, include = TRUE}
...
```{r all_ps_const, fig.height = 7, fig.width=20, meassage = F, warning=F}
...
Weighted means by the probability of presence within the area is then specific to that area, which means the DECIDE score wouldn't be comparable across the whole UK. Could this be a problem from the project aims perspective? Weighted by max probability of presence means that species that are most likely to be present in an area will be upweighted compared to species that are less likely. This might not be the best idea because it means that common species are likely to be upweighted across the whole UK. I.e. common species are likely to have high maximum probability of presence across much of the UK. A slight alteration of this might be a good way to increase the importance of habitat specialists, but would need to account for number of grid cells within a specific area with a high probability of presence rather than maximum.  
...
If we are interested in improving the models the most, this will probably be by sending people to areas with the highest uncertainty. This will of coourse influence how likely a recorder is to see a species which might be a problem.
...
Is it better just to take the mean across all cells, without any weightings? This means that each species will be given the same weighting in the final value of the cell. If we take the geometric mean it reduces the influence of outliers which could be good. Conversely, it might be better to have the DECIDE score be strongly influenced by outliers, in which case, the arithmetic mean might be better. 
...
```{r extra_plots, include = T, warning = F, message = F}
