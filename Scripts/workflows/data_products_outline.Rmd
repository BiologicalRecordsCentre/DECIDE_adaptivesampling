...
**What will the DECIDE score used to develop nudges be based on?**
...
```{r aggregatedmetric, fig.height = 6, fig.width = 12, fig.cap='The summed DECIDE score across all species'}
...
We can then extract the  metric layer into areas that are accessible to recorders, for example public footpaths and greenspaces (Figure \@ref(fig:plotaccmetric))
...
suppressWarnings(
  access_metrics <- lapply(final_acc_loc[c(1:2, 4)], extract_metric, metric = sf_rast))

```

```{r plotaccmetric, fig.cap='Public Rights of Way and Accessible Greenspaces with their associated metric'}

ggplot() +
  geom_sf(data = sf_rast, aes(fill = error_metric), alpha = 0.5, colour = 'white') +
  geom_sf(data = access_metrics[[3]], aes(fill = error_metric)) +
