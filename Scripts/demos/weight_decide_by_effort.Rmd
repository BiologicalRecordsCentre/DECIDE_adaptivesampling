...
So, we probably want time since the last record, irrespective of the number of visits. So, just time since the last record as the way of creating a weighting to affect the decide score.

This function can now produce two outputs. The first is just the number of records in a cell across all years. The second, is the number of years since the most recent record.
                          ...
                          weight_by_time = TRUE)
{
  ...
  if(weight_by_time){
    ...
    record_weights <- record_counts %>% 
      group_by(lon, lat) %>% 
      dplyr::summarise(lst_rec = max(year), # what is the most recent year?
                       score = year(Sys.Date())-lst_rec) %>%  # number of years since the last record in a grid cell
      ungroup()
...
plot(rec_counts, main = 'records weighted by time')
