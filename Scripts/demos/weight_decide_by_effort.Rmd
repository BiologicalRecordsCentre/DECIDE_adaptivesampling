...
# crop
records <- filter_distance(st_dfm,
                           distance = 5000)
                          ...
                          template_raster) # raster that you want the counts to be aggregated by
{
  ...
  spdf.counts <- SpatialPointsDataFrame(coords = xy, data = record_counts,
  ...
  counts = table(cellFromXY(n_recs,spdf.tax))
...
count_records(records_df = records, template_raster = dec_crop)

#### function does what is below

# store counts as a data.frame
rec_count <- records %>% 
  mutate(lon = unlist(map(records$geometry, 1)),
         lat = unlist(map(records$geometry, 2))) %>% 
  group_by(lon, lat) %>% 
  tally %>% as.data.frame() %>% 
  mutate(geometry = NULL)
rec_count

xy <- rec_count[,c("lon","lat")]
spdf.tax <- SpatialPointsDataFrame(coords = xy, data = rec_count,
                                   proj4string = CRS("+init=epsg:27700")) 
