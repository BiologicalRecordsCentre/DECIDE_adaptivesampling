...
This might not work if the data and the raster are at different resolutions. Very slow on large rasters. Might be possible to do this in package sf rather than relying on raster. Only possible if the coords of the data frame and raster are the same.

We need a way of downweighting cells that have lots of sightings in them. I have generated this below for weight_by_time = FALSE.
...
Other option is take the number of records in a cell and the number of years that have passed since those records were made. Divide the number of years since the last record by the number of records and use that as a score... 

Other option is to take the number of years since the most recent record and divide it by the number of records in that year. This means that grid cells with few records from a long time ago would be the most upweighted. Grid cells visited recently with most records would be the most downweighted. The problem is that this treats records in the same way as the number of years since the last record and means that, for e.g., that 1 record in 2013 [(2021-2013)/1=8] has a higher weighting than 5 records in 2011 [(2021-2011)/5=2]. Which we don't want. 

So, we probably want time since last record irrespective of number of visits. So, just time since the last record as the way of weighting 
                          ...
                          weight_by_time = TRUE)
{
  ...
  if(weight_by_time){
    ...
    record_weights <- record_counts %>% 
      ...
      dplyr::summarise(n = length(year[year == max(year)]), # number of records in the grid cell in the most recent year
                       lst_rec = max(year), # what is the most recent year?
                       yrs_since_last_rec = year(Sys.Date())-lst_rec, # number of years since the last record
                       score = yrs_since_last_rec/n) %>% # number of years since the last record / the number of records in that year. 
    # dplyr::summarise(n = n(),
    #                  yrs_since_pres = year(Sys.Date())-year,
    #                  score = yrs_since_pres/n) %>% #/rnk) %>% 
    distinct() %>% 
      group_by(lon, lat) %>% 
...
record_counts %>% 
  group_by(lon, lat) %>% 
  dplyr::summarise(n = length(year)[year == max(year)],
                   lst_rec = max(year),
                   yrs_since_last_rec = year(Sys.Date())-lst_rec,
                   score = yrs_since_last_rec/n) %>% #/rnk) %>% 
  distinct()

t <- record_counts[record_counts$lon == 345750 & record_counts$lat == 680450, ]
t

max(t$year)
length(t$year[t$year == max(t$year)])

t %>% 
  dplyr::summarise(n = length(year[year == max(year)]),
                   lst_rec = max(year),
                   yrs_since_last_rec = year(Sys.Date())-lst_rec,
                   score = yrs_since_last_rec/n) 
                             ...
                             recording_impact = 10) # the larger the value the less impact it has
{
  ...
  weighting <- 1/(1+(weighting/recording_impact))
  ...
  return(list(smoothed_effort = smoothed_effort,
...
par(mfrow = c(1,2))
plot(dec_crop, main = 'Raw variation')
