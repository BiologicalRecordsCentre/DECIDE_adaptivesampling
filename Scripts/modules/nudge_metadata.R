...
access_sub <- access[lengths(access) != 0]


# get each layer buffered
shapes_list <- lapply(access_sub, FUN = function(x) ((st_buffer(x, buffer))))


lapply(1:dim(tn_df)[1], FUN = function(n) { # I want to find which shapes each point is in
  
  out <- lapply(1:length(shapes_list), FUN = function(x){
